<?php
	...
	function endGame() {
		...
		$str = '{ "jackpot_id": "'.$jackpot_id.'", "winner_64": "'.$winner_steam_64.'" }';
		echo $str;
	}
	...
	function addItem($user_64, $items, $trade_id) {
		...
		$add_items_to_jackpot_query = 'INSERT INTO jackpot_item (item_id, item_jackpot_id, item_user_64, item_class_id, item_context_id, item_asset_id, item_steam_instance, item_name, item_price, item_collection) VALUES';
		...
		foreach ($items as $item) {
			...
			$add_items_to_jackpot_query.=' (NULL, "'.$jackpot_id.'", "'.$trade_steam_64.'", "'.$class.'", "'.$contextid.'", "'.$assetid.'", "'.$instance.'", "'.$item_name.'", "'.$item_price.'", "'.$random_id.'"), ';
		}
	...
	function createGame() {
		...
		// $last_game_result = mysqli_query($dbc, 'SELECT jackpot_id, jackpot_winner_64 FROM jackpot ORDER BY jackpot_id DESC LIMIT 1;');
		// $row = mysqli_fetch_assoc($last_game_result);
		// $jackpot_status = $row['jackpot_winner_64'];
		// if ($jackpot_status == '') {
